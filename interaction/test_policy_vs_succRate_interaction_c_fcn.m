function [succ_meas_diff_set, kine_meas_diff_set, varargout] = test_policy_vs_succRate_interaction_c_fcn(score_succ_by_trial_grp, score_fail_by_trial_grp)
...
for i_grp = 1:4
    try
    temp_succ = score_succ_by_trial_grp{1, i_grp}(:, :, probe_day_by_grp{i_grp});
    temp_fail = score_fail_by_trial_grp{1, i_grp}(:, :, probe_day_by_grp{i_grp});
    kine_meas{1, i_grp} = nan(size(temp_succ));
    catch
        warning('wtf')
    end
    for i_sub = 1:size(temp_succ,2)
        temp_union = nan(size(temp_succ,1),1);
        for i_tr = 1:size(temp_succ,1)
            temp_trial = [temp_succ(i_tr, i_sub), temp_fail(i_tr, i_sub)];
            if sum(isnan(temp_trial)) < 2
                if sum(isnan(temp_trial)) == 0
                    % both not nan?? shouldn't happen. Make nan
                    temp_union(i_tr) = nan;
                    warning('Weird: Both succ and fail have kin val.');
                else
                    % exactly one is nan, just like it should be.
                    temp_union(i_tr) = temp_trial(~isnan(temp_trial));
                end
            else
                % neither succ nor fail has a value; not great but fine.
                temp_union(i_tr) = nan;
            end
        end
        kine_meas{1, i_grp}(:, i_sub) = temp_union; %order-preserved union
    end
    
    temp_succ = score_succ_by_trial_grp{2, i_grp}(:, :, probe_day_by_grp{i_grp});
    temp_fail = score_fail_by_trial_grp{2, i_grp}(:, :, probe_day_by_grp{i_grp});
    kine_meas{2, i_grp} = nan(size(temp_succ));
    for i_sub = 1:size(temp_succ,2)
        temp_union = nan(size(temp_succ,1),1);
        for i_tr = 1:size(temp_succ,1)
            temp_trial = [temp_succ(i_tr, i_sub), temp_fail(i_tr, i_sub)];
        ...
        kine_meas{2, i_grp}(:, i_sub) = temp_union; %order-preserved union
    end
end
...
kin_frac = nan(4, 21);
for i_grp = 1:4
%     succ_frac(i_grp, 1:size(succ_meas{1, i_grp},2)) = nanmean((succ_meas{2, i_grp})-(succ_meas{1, i_grp})./(succ_meas{1, i_grp}));
%     kin_frac(i_grp, 1:size(succ_meas{1, i_grp},2)) = nanmean((kine_meas{2, i_grp})-(kine_meas{1, i_grp})./(kine_meas{1, i_grp}));
try
    succ_frac(i_grp, 1:size(succ_meas{1, i_grp},2)) = nanmean(succ_meas{2, i_grp})./nanmean(succ_meas{1, i_grp});
    kin_frac(i_grp, 1:size(kine_meas{1, i_grp},2)) = nanmean(kine_meas{2, i_grp})./nanmean(kine_meas{1, i_grp});
catch
    warning('wtf')
end
end
varargout = {succ_frac, kin_frac};
