...
for i_grp = 2:4
    ...
    pr_loc_gvn_succ_vLEARN_successes_v5;
...
%     score_fail_by_trial_grp{2, i_grp} = score_by_trial_prb;
    ...
    pr_loc_gvn_succ_vLEARN_failures_v5;
...
x = reshape([pre_wind_wind(1:grp_days(4)); prb_wind_wind(1:grp_days(4))], 2*grp_days(4), 1);
...
% within day analysis:
resp = [reshape(score_succ_all, 60*20, 1); reshape(score_fail_all, 60*20, 1)]; %total_subjects x windows
