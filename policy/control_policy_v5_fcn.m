function [tilt_dir, traj_dir, tilt_mag, traj_loc] = control_policy_v5_fcn(len_range)
...
% n_samp = 20;%10
n_samp = round(100*(len_range(2) - len_range(1)));
...
% th_1 = .25; th_2 = .6;
th_1 = len_range(1); th_2 = len_range(2);
...
% direction and tilt magnitude (each of the last two at 150ms prior) at
...
%                     tilt_dir_all = tilt_d((intx_ind - n_samp_delay):end);
                    tilt_dir_filt = sgolayfilt(tilt_d, 3, 5);
%                     if intx_ind - n_samp_delay > 0
...
%                     trial_dt{i_grp}(i_tr, i_sub) = dist_t;
                catch
                    intx_ind = nan;
                end
            end
